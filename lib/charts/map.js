...
class MapTransformer extends BaseTransformer {
    ...
    transform(data) {
        ...
        // Process data points
        const { normalPoints, effectPoints } = this.processDataPoints(data, config);
     ...
     */
    processDataPoints(data, config) {
        const normalPoints = [];
        const effectPoints = [];

        data.forEach(item => {
            const point = this.parseDataItem(item, config);
