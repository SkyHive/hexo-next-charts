...
class StoreAdapter {
    ...
    load() {
        ...
        if (fs.existsSync(this.filePath)) {
            try {
                ...
                // Migrate old format (simple key-value) to new schema if necessary
                if (!json.store && !json.aliases) {
                    // It's the old format: {"City": [lng, lat]}
                    // We treat them as confirmed store entries with manual source
                    this.data.store = {};
                    this.data.aliases = {};
                    for (const [key, coords] of Object.entries(json)) {
                        const id = key.toLowerCase().replace(/\s+/g, '_');
                        this.data.store[id] = {
                            name_cn: key, // Best guess
                            coords: coords,
                            source: 'manual' 
                        };
    ...
    /**
     * Check if a name is likely an IATA code (3 uppercase letters)
     */
    isIata(name) {
