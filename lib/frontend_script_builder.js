...
<script>
  ...
  window.HEXO_NEXT_CHARTS_DATA.places = Object.assign(window.HEXO_NEXT_CHARTS_DATA.places || {}, ${placesJson});
     ...
     */
    getEChartsUrl() {
        const userConfig = this.hexo.config?.next_charts?.echarts;
        if (userConfig?.cdn) {
            return userConfig.cdn;
        }
        return 'https://cdn.jsdelivr.net/npm/echarts@6.0.0/dist/echarts.min.js';
    }
    ...
    getMapCdnFallback() {
        ...
        if (userConfig?.map_cdn) {
            return { ...defaultFallback, ...userConfig.map_cdn };
        }
    ...
    buildLoaderScript(mapDataPaths) {
        ...
        const cdnFallback = this.getMapCdnFallback();
        const mapDataPathsJson = JSON.stringify(mapDataPaths);
        const cdnFallbackJson = JSON.stringify(cdnFallback);
...
(function() {
    ...
    const cdnFallback = ${cdnFallbackJson};
